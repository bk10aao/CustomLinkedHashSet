...
(excluding removeAll, retainAll and constructors).
...
# Load datasets
v2_df  = load_wide_jmh_csv('../CustomLinkedSet_jmh_performanceV2.csv')   # Custom V2
jdk_df = load_wide_jmh_csv('../LinkedHashSet_jmh_performance.csv')       # JDK
...
common_sizes = sorted(list(set(v2_pivot.index).intersection(set(jdk_pivot.index))))
...
benchmarks = [
    b for b in v2_pivot.columns
    if b in jdk_pivot.columns
]
...
v2_win_color  = '#4DA6FF'   # Blue  → V2 faster
jdk_win_color = '#FF4D4D'   # Red   → JDK faster

for b in benchmarks:
    v2_vals  = v2_fixed[b].dropna()
    jdk_vals = jdk_fixed[b].dropna()
    if v2_vals.empty or jdk_vals.empty:
        continue

    # JDK time / V2 time  →  >1 means V2 is faster
    per_size_ratios = jdk_vals / v2_vals
...
ax.set_title(
    'Overall Relative Performance Comparison (CustomLinkedSet V2 vs JDK LinkedHashSet)\n'
    '(Geometric Mean Across All Sizes – removeAll / retainAll / Constructors excluded)',
    fontsize=14, fontweight='bold', pad=15, color='#ffffff'
)

ax.set_xlabel(
    '← JDK Faster  |  Relative Speedup Factor (Uncapped)  |  V2 Faster →',
...
plt.tight_layout()
plt.savefig('geometric_v2_vs_jdk.png', dpi=300, transparent=True, bbox_inches='tight')
plt.close()

print('Generated geometric comparison graph (V2 vs JDK, outliers excluded) successfully!')