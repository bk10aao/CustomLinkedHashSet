...
CustomLinkedSet V1, CustomLinkedSet V2 and JDK LinkedHashSet
...
V1_CSV_PATH  = "CustomLinkedSet_jmh_performanceV1.csv"  # Custom V1
...
def create_chart(col_name, chart_title, v1_data, v2_data, jdk_data,
                 canonical_sizes, output_path):

    v1_values = [
        v1_data[s][col_name] if s in v1_data and col_name in v1_data[s] else np.nan
    ...
    # Lines
    ax.plot(x_positions, v1_values,  color=COLORS['blue'],   linewidth=1.5, zorder=2)
    ...
    # Markers
    ax.scatter(x_positions, v1_values,  color=COLORS['blue'],   s=35, marker='o',
    ...
    legend_elements = [
        Line2D([0], [0], marker='o', color='none',
               ...
               markeredgewidth=1.5, markersize=8, label='V1', linestyle='none'),
        Line2D([0], [0], marker='o', color='none',
...
def main():
    for path in (V1_CSV_PATH, V2_CSV_PATH, JDK_CSV_PATH):
    ...
    print(f"Loading {V1_CSV_PATH} (V1)...")
    v1_data = load_wide_jmh_csv(V1_CSV_PATH)
    ...
    canonical_sizes = sorted(
        list(set(v1_data.keys()) | set(v2_data.keys()) | set(jdk_data.keys()))
    )
    ...
    for file_slug, col_name in OPERATIONS.items():
        ...
        create_chart(col_name, col_name, v1_data, v2_data, jdk_data,
                     canonical_sizes, output_path)
